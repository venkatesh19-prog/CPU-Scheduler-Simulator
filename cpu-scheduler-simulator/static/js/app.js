...
if (window.__schedulerAppLoaded) {
  ...
  (function () {
    ...
    const updateRec = () => {
      ...
      const recAlgo = $("#recAlgo");
      const recReason = $("#recReason");
      const recQuantum = $("#recQuantum");
      if (recAlgo) recAlgo.innerText = rec.algo;
      if (recReason) recReason.innerText = rec.reason;
      if (recQuantum) recQuantum.innerText = suggestQuantum(processes);
    };
    ...
    (function () {
      ...
      const setCanvasHeightForRows = (rowsCount) => {
      ...
      const renderStatic = (schedule, meta) => {
        resizeCanvas();
        ...
        pidRows.forEach((row) => {
          ...
          ctx.fillText(row.pid, 8, y + trackH / 2 + 2);
          ctx.fillStyle = "#f8fafc";
          const x0 = padding.left;
          const w = anim.width - padding.left - padding.right;
        ...
        schedule.forEach((seg) => {
          ...
          const x2 = timeToX(e, start, end, anim.width);
          const w = Math.max(1, x2 - x1);
          ctx.fillStyle = pid ? row.color : "#e5e7eb";
          ctx.globalAlpha = pid ? 1 : 0.9;
          roundRect(ctx, x1, y + 2, w, trackH - 4, radius);
          if (w > 40) {
            ctx.fillStyle = "#fff";
            ctx.font = "600 11px Arial";
            ctx.textAlign = "center";
            ctx.fillText(pid ? `${pid} (${s}-${e})` : "idle", x1 + w / 2, y + trackH / 2 + 2);
          } else {
            ctx.fillStyle = "#0b1220";
            ctx.font = "11px Arial";
            ctx.textAlign = "left";
            ctx.fillText(pid || "idle", x2 + 6, y + trackH / 2 + 2);
          }
        });
      ...
      const renderFrame = () => {
        ...
        if (active) {
          ...
          const x2 = timeToX(e, anim.start, anim.end, anim.width);
          const w = Math.max(1, x2 - x1);
        ...
        ctx.lineTo(executedX + 0.5, anim.height - padding.bottom + 6);
        ctx.stroke();
        ctx.fillStyle = "#0b1220";
        ctx.font = "600 12px Arial";
        ctx.textAlign = "center";
        ctx.fillText(anim.current.toFixed(2), executedX, padding.top + 2);
      };

      const showPopup = (pid, start, burst) => {
        ...
        const pidEl = $("#popupPid");
        const metaEl = $("#popupMeta");
        if (pidEl) pidEl.innerText = pid;
        if (metaEl) metaEl.innerText = `Burst: ${burst}  •  Start: ${start}`;
      ...
      if (addRandom) {
        addRandom.addEventListener("click", () => {
          for (let i = 0; i < 5; i++) addProcess(Math.floor(Math.random() * 8), Math.floor(Math.random() * 6) + 1);
        });
      }
    ...
    function updateUI() {
      ...
      if (tickLabel) tickLabel.innerText = `Tick: ${tick}/${metrics ? metrics.totalTime : 0}`;
      const sb = $("#summaryBox");
      if (!metrics) {
        if (sb) sb.innerHTML = "No data";
      } else {
        if (sb) {
    ...
    function addProcess(a, b) {
      const id = nextId++;
      const p = { id, pid: `P${id}`, arrival: Number(a), burst: Number(b), remaining: Number(b), color: randColor(id) };
      processes.push(p);
      processes.sort((x, y) => x.arrival - y.arrival || x.id - y.id);
      recompute();
    }
    ...
    function wire() {
      ...
      const addProcEl = $("#addProc");
      const addSampleEl = $("#addSample");
      const stepBack = $("#stepBack");
      const stepForward = $("#stepForward");
      const resetBtn = $("#resetBtn");

      modeEl && modeEl.addEventListener("change", (e) => {
        mode = e.target.value;
        recompute();
      });
      goalEl && goalEl.addEventListener("change", (e) => {
        goal = e.target.value;
        recompute();
      });
      predictEl && predictEl.addEventListener("change", (e) => {
        enablePrediction = e.target.checked;
        recompute();
      });
      addProcEl && addProcEl.addEventListener("click", () => {
      ...
      addSampleEl && addSampleEl.addEventListener("click", () => {
        [{ arrival: 0, burst: 5 }, { arrival: 2, burst: 3 }, { arrival: 4, burst: 1 }].forEach((s) => addProcess(s.arrival, s.burst));
      });
      $("#loadSimple") && $("#loadSimple").addEventListener("click", () => { /* handled in canvas module - safe */ });
      $("#addRandom") && $("#addRandom").addEventListener("click", () => { /* handled in canvas module - safe */ });

      stepBack && stepBack.addEventListener("click", () => {
        if (!metrics) return;
        tick = Math.max(0, tick - 1);
        updateUI();
      });
      stepForward && stepForward.addEventListener("click", () => {
        if (!metrics) return;
        tick = Math.min(metrics.totalTime, tick + 1);
        updateUI();
      });
      ...
      $("#playSidebar") &&
        $("#playSidebar").addEventListener("click", () => {
          ...
          if (!metrics) return;
          if (sp._int) {
            clearInterval(sp._int);
            sp._int = null;
            sp.innerText = "Play";
          } else {
            sp._int = setInterval(() => {
              tick++;
              if (tick >= metrics.totalTime) {
                clearInterval(sp._int);
                sp._int = null;
                sp.innerText = "Play";
                tick = metrics.totalTime;
              }
              updateUI();
            }, 250);
            sp.innerText = "Pause";
